...
- 支持按基金名称、6 位代码或拼音缩写联想搜索并添加基金。
- 支持将自选、持仓和交易记录保存到后端 SQLite，并通过可切换、排序和删除的命名分类在多个设备间同步。
- 展示基金名称、类型、最新正式净值和可切换区间、支持悬停/触摸查看的历史净值图。
- 获取最近一期公开股票持仓。
- 获取 A 股和港股行情；港股估值同时考虑港币兑人民币变化。
- 展示预测涨跌、预测净值、行情覆盖率、置信度和单只持仓贡献。
- 每只基金可按份额与成本净值录入持仓，也可填写总持仓金额和盈亏金额并通过最新正式净值反推；按盘中预计净值或盘后正式净值计算总盈利与盈利率。
- 支持记录买入和卖出，默认今日并可选择历史日期，按金额或交易前持仓比例使用正式净值换算份额，以移动平均法更新成本并支持撤销最近一笔。
- 支持将当前浏览器的自选、持仓与交易记录导出为带版本号的 JSON 文本，并通过合并或覆盖方式导入。
- 手机、平板、PC 响应式布局。

## 后端同步

- 数据默认保存在 `data/funding-rader.db`，重启服务不会丢失。
- 可通过 `FUNDING_DATA_DIR=/path/to/persistent-volume` 指定持久化目录。
- 当前没有用户和权限管理，所有能访问网站的人都可以查看及修改全部分类。
